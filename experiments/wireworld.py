...
class Cell:
    def __init__(self, x, y, w):
        ...
        for (i, _) in enumerate(self.pre):
            if i < 2:
                tl.add(Reset(self.pre[i+1]), on=_)
    ...
    def on_head(self):
        base, wire, tail, head = self.base, self.wire, self.tail, self.head
        ...
        tl.add(Appear(self.delete), on=head)
        tl.add(Disappear(self.delete), on=self.delete)
        tl.add(Disappear(tail), on=self.delete)
